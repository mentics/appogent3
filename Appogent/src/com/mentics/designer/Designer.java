...
import static com.mentics.designer.BasicNodePainter.selectedLabels;
...
public class Designer extends Application {
    ...
    public void start(Stage stage) throws Exception {
        mgr = new ModelManager<>(new DataModel("<New Project>", IntTreePMap.<Item>empty()), threads);
    ...
    private Scene createScene() {
        ...
        text.setTranslateY(text.getBoundsInLocal().getHeight());

        SodiumFX.bind(text.textProperty(), mgr.value.map(DataModel.TO_PROJECT_NAME));
        ...
        //System.out.println(PerformanceTracker.getSceneTracker(actOn.getScene()).getInstantFPS());
        return scene;
    }
        ...
        pane.addEventHandler(KeyEvent.KEY_PRESSED, new EventHandler<KeyEvent>() {
            ...
            public void handle(KeyEvent ev) {
                if (ev.getCode() == KeyCode.INSERT) {
                    ...
                    mgr.send(new AddNodeAction(new Item(++counter, name)));
                }
            }
        });
    ...
    public Node findNearestNode(String code) {
        Node selectedNode = BasicNodePainter.currentLabel.get();
        ...
        for (Node PTmp : group.getChildren()) {
    ...
    public boolean isInDirection(String direction, Point2D currentPosition, Node node) {
        boolean ret = false;
        switch (direction) {
            case "LEFT":
